...
const ALL_CATEGORIES = [
  { id: "technology", name: "Technology", color: "bg-blue-500", selected: true },
  ...
  { id: "entertainment", name: "Entertainment", color: "bg-pink-500", selected: false },
  { id: "sports", name: "Sports", color: "bg-orange-500", selected: false },
  { id: "politics", name: "Politics", color: "bg-amber-500", selected: false },
  { id: "environment", name: "Environment", color: "bg-emerald-500", selected: false },
]
...
export function CategoryManager() {
  ...
  const toggleCategory = (id) => {
    setCategories(categories.map((cat) => (cat.id === id ? { ...cat, selected: !cat.selected } : cat)))
  }
    ...
    <Card className="bg-zinc-900/50 border-zinc-800">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg flex items-center justify-between">
          ...
          <span className="text-sm text-zinc-400 font-normal">{selectedCount}/4 selected</span>
        </CardTitle>
      </CardHeader>
          ...
          {categories.map((category) => (
            <Button
              ...
              className={`justify-start border-zinc-800 hover:border-zinc-700 ${
                category.selected ? "bg-zinc-800/50 text-white" : "bg-transparent text-zinc-400"
              } ${selectedCount >= 4 && !category.selected ? "opacity-50 cursor-not-allowed" : ""}`}
              onClick={() => {
            ...
            >
              ...
              {category.selected ? <X className="h-4 w-4 text-zinc-400" /> : <Plus className="h-4 w-4 text-zinc-400" />}
            </Button>
          ))}
        </div>
